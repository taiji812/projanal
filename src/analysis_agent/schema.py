...
class ModuleForm(BaseModel):
    form_type: str = "sourcecode"       # sourcecode | binary | script
    source_language: list[str] = Field(default_factory=list)
    has_submodule: bool = False


class FunctionalFeatures(BaseModel):
    capabilities: list[str] = Field(default_factory=list)   # command and control | execution | ...
    post_exploits: list[str] = Field(default_factory=list)  # keylogger | file upload | ...


class BuildParam(BaseModel):
    param_type: str = "string"                  # string | choice | object | boolean
    param_title: str = ""
    param_name: str
...
class BuildEnv(BaseModel):
    build_env_os: str = "unknown"   # windows | linux | macos | cross
...
class BuildFeatures(BaseModel):
    ...
    build_env: BuildEnv = Field(default_factory=lambda: BuildEnv(build_env_os="unknown"))
...
class ExecutionComponent(BaseModel):
    name: str
    exec_arch: list[str] = Field(default_factory=list)      # x86 | x64 | arm64
    exec_os: list[str] = Field(default_factory=list)        # windows | linux | macos
    exec_type: list[str] = Field(default_factory=list)      # exe(.net) | dll(native) | ...
...
class KeyInfo(BaseModel):
    ...
    key_input_type: Optional[str] = None    # user-input | embedded
...
class PayloadInfo(BaseModel):
    ...
    execution_method: list[str] = Field(default_factory=list)      # reflective-loading | ...
    execution_technique_notes: list[str] = Field(default_factory=list)
    payload_target_path: Optional[str] = None
    delivery_type: Optional[str] = None         # external-file | embedded
...
class ModuleProperty(BaseModel):
    module_name: str
    module_category: str = ""   # RAT | Loader | Implant | Credential Harvester | ...
